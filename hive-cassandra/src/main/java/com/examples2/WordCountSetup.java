...
public class WordCountSetup {
	...
	private static void setupKeyspace(Cassandra.Iface client) throws InvalidRequestException, UnavailableException, TimedOutException, SchemaDisagreementException, TException {
		KsDef ks;
		try {
	...
	private static void setupTable(Cassandra.Iface client) throws InvalidRequestException, UnavailableException, TimedOutException, SchemaDisagreementException, TException {
		String query = "CREATE TABLE " + WordCount.KEYSPACE + "."+ WordCount.COLUMN_FAMILY + 
				" ( id uuid," + "   line text, " + "   PRIMARY KEY (id) ) ";
		...
		query = "CREATE TABLE " + WordCount.KEYSPACE + "." + WordCount.OUTPUT_COLUMN_FAMILY + 
				" ( word text," + "   count_num text," + "   PRIMARY KEY (word) ) ";
	...
	private static void insertData(Cassandra.Iface client) throws InvalidRequestException, UnavailableException, TimedOutException, SchemaDisagreementException, TException {
	...
	private static String[] bodyData() { // Public domain context, source  http://en.wikisource.org/wiki/If%E2%80%94
		return new String[] { 
