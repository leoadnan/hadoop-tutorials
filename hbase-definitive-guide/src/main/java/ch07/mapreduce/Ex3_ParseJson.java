...
// MapReduce job that parses the raw data into separate columns.
...
public class Ex3_ParseJson {

	private static final Log LOG = LogFactory.getLog(Ex3_ParseJson.class);

	public static final String NAME = "Ex3_ParseJson";
	...
	static class ParseMapper extends
			TableMapper<ImmutableBytesWritable, Mutation> {
			...
			try {
				...
				for (Cell cell : columns.listCells()) {
					...
					for (Object key : json.keySet()) {
						...
						// 1-Put Store the top-level JSON keys as columns, with their value set as the columnvalue.
	...
	public static void main(String[] args) throws Exception {
		...
		if (column != null) {
			...
			if (colkey.length > 1) {
				...
				// 2-Conf Store the column family in the configuration for later use in the mapper.
		...
		job.setJarByClass(Ex3_ParseJson.class);
		
		// 3-SetMap Setup map phase details using the utility method.
		TableMapReduceUtil.initTableMapperJob(input, scan, ParseMapper.class, ImmutableBytesWritable.class, Put.class, job);
		
		// 4-SetReduce Configure an identity reducer to store the parsed data.
