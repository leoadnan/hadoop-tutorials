...
public class Ex1_ExampleSetup {
	...
	/**
	 * Generates a row key that contains an MD5 hash of the specified user ID
	 * and a random date in the year 2014 but with a random month, day, and time
	 *
	...
	/**
	 * Extracts the date from a row key. Note that the user ID is hashed
	 * (one-way operation) so there is no way to retrieve the user ID, but you
	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < 10000; i++) {
				...
				put.addColumn(Bytes.toBytes("info"), Bytes.toBytes("page"), Bytes.toBytes("/page/" + i % 100));
