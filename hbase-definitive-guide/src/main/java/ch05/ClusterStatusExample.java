...
//Example reporting the status of a cluster
public class ClusterStatusExample {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		// 1-GetStatus Get the cluster status.
		...
		System.out.println("\nServer Info:\n--------------");
		// 2-ServerInfo Iterate over the included server instances.
		for (ServerName server : status.getServers()) {
			...
			// 3-ServerLoad Retrieve the load details for the current server.
			...
			System.out.println("\nRegion Load:\n--------------");
			// 4-Regions Iterate over the region details of the current server.
			for (Map.Entry<byte[], RegionLoad> entry : load.getRegionsLoad().entrySet()) {
				...
				// 5-RegionLoad Get the load details for the current region.
