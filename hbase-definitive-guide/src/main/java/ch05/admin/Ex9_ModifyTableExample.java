...
//Example modifying the structure of an existing table
public class Ex9_ModifyTableExample {
	...
	public static void main(String[] args) throws IOException,
		...
		HTableDescriptor desc = new HTableDescriptor(tableName)
			.addFamily(coldef1)
			.setValue("Description", "Chapter 5 - Ex9_ModifyTableExample: Original Table");

		// 1-CreateTable Create the table with the original structure and 50 regions.
		admin.createTable(desc, Bytes.toBytes(1L), Bytes.toBytes(10000L), 50);

		// 2-SchemaUpdate Get schema, update by adding a new family and changing the maximum file size property.
		...
		htd1.addFamily(coldef2)
				...
				.setValue("Description","Chapter 5 - Ex9_ModifyTableExample: Modified Table");

		admin.disableTable(tableName);

		// 3-ChangeTable Disable and modify the table.
		admin.modifyTable(tableName, htd1);

		// 4-Pair Create a status number pair to start the loop.
		...
		// 5- Loop over status until all regions are updated, or 500 seconds have been exceeded.
		...
		// 6-Verify Check if the table schema matches the new one created locally.
