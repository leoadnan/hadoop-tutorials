...
public class FamilyFilterExample {
	...
	public static void main(String[] args) throws IOException {
		...
		// 2-Scan Scan over table while applying the filter.
		...
		// 3-Get Get a row while applying the same filter.
		Result result1 = table.get(get1);
		System.out.println("Result of get(): " + result1);

		Filter filter2 = new FamilyFilter(CompareFilter.CompareOp.EQUAL, new BinaryComparator(Bytes.toBytes("colfam3")));
		// 4-Mismatch Create a filter on one column family while trying to retrieve another.
		Get get2 = new Get(Bytes.toBytes("row-5"));
		get2.addFamily(Bytes.toBytes("colfam1"));
		get2.setFilter(filter2);
		
		// 5-Get2 Get the same row while applying the new filter, this will return "NONE".
