...
public class Ex6_BufferedMutatorExample {

	private static final Log LOG = LogFactory.getLog(Ex6_BufferedMutatorExample.class);
	...
	public static void main(String[] args) throws Exception {
		...
		BufferedMutator.ExceptionListener listener = new BufferedMutator.ExceptionListener() {
			...
			public void onException(RetriesExhaustedWithDetailsException e, BufferedMutator mutator) {
				// 02- Handle callback in case of an exception.
				for (int i = 0; i < e.getNumExceptions(); i++) {
					// 03- Generically retrieve the mutation that failed, using the common superclass.
					LOG.info("Failed to sent put: " + e.getRow(i));
				}
			}
		};

		// 04- Create a parameter instance, set the table name and custom listener reference.
		BufferedMutatorParams params = new BufferedMutatorParams(TABLE).listener(listener);

		try (
		// 05- Allocate the shared resources using the Java 7 try-with-resource pattern.
		Connection conn = ConnectionFactory.createConnection(configuration);
		BufferedMutator mutator = conn.getBufferedMutator(params)) {

			// 06- Create a worker pool to update the shared mutator in parallel.
			...
			for (int i = 0; i < TASK_COUNT; i++) {
				futures.add(workerPool.submit(new Callable<Void>() {
					...
					public Void call() throws Exception {
						...
						// 08- Each worker uses the shared mutator instance, sharing the same backing buffer, callback listener, and RPC execuor pool.
						mutator.mutate(p);
						// [...]
			...
			for (Future<Void> f : futures) {
				// 09- Wait for workers and shut down the pool.
				f.get(5, TimeUnit.MINUTES);
			}
			workerPool.shutdown();
		} catch (IOException e) {
			// 10- The try-with-resource construct ensures that first the
			// mutator, and then the connection are closed. This could trigger
