...
public class Ex1_DeleteExample {
	...
	public static void main(String[] args) throws IOException {
		...
		// 1-NewDel Create delete with specific row.
		Delete delete = new Delete(Bytes.toBytes("row1"));

		// 2-SetTS Set timestamp for row deletes.
		delete.setTimestamp(1);

		// 3-DelColNoTS Delete the latest version only in one column.
		delete.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("qual1"));
		
		// 4-DelColTS Delete specific version in one column.
		delete.addColumn(Bytes.toBytes("colfam1"), Bytes.toBytes("qual3"), 3);

		// 5-DelColsNoTS Delete all versions in one column.
		delete.addColumns(Bytes.toBytes("colfam1"), Bytes.toBytes("qual1"));
		
		// 6-DelColsTS Delete the given and all older versions in one column.
		delete.addColumns(Bytes.toBytes("colfam1"), Bytes.toBytes("qual3"), 2);

		// 7-AddCol Delete entire family, all columns and versions.
		delete.addFamily(Bytes.toBytes("colfam1"));
		
		// 8-AddCol Delete the given and all older versions in the entire column family, i.e., from all columns therein.
